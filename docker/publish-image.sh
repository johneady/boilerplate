#!/usr/bin/env bash
# Builds the runtime image locally and pushes it to GHCR, as a stand-in for
# .github/workflows/docker.yml when Actions minutes are unavailable.
#
# This is a TEMPORARY fallback. It exists so the deploy path stays exactly the
# same -- Dokploy still pulls an immutable `sha-<full-sha>` tag from GHCR, it
# just was not CI that put it there. Nothing about docker-compose.dokploy.yml
# or the Dokploy environment changes. Delete this script once the workflow can
# run again; do not let it become the normal way images get published.
#
