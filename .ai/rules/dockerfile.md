...
**The repo's Actions need Write access on the package itself — `packages: write` in the workflow is not enough.** The package was first created by a local `publish-image.sh` push (a PAT, not Actions), and a package created outside Actions grants the repo's `GITHUB_TOKEN` nothing, even though it shows as linked to `johneady/boilerplate`. Every non-PR run then fails at push with `denied: permission_denied: read_package` (note *read*, not *write*) while PR builds stay green because they never push — that went unnoticed from 2026-09-17 to 2026-09-26. Fix is UI-only (no REST endpoint): package settings → **Manage Actions access** → add the repo with role **Write** (the default Read still fails). Do not "fix" it by deleting the package so CI recreates it: that discards the `sha-…` tags Dokploy is pulling.
