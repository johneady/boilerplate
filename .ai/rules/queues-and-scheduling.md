...
The staged original is cleaned up on every exit: handle() deletes it on success, and failed() deletes it (plus anything a killed attempt half-wrote) once the retries are exhausted. The OTHER leak -- a dispatch that never happened because the queue was down at the moment of staging -- has no job to clean up after it, so app:prune-orphaned-media also sweeps App\Media\StagedUpload::DIRECTORY past the same retention window.
...
The adopted row is OWNERLESS, deliberately. What owns a body image is the body text naming it, not a record: several pages may share one file, and a row attached to one of them would be deleted -- files and all -- with that page, breaking every other page still rendering it. app:prune-orphaned-media spares rows a page body still names and collects one once no body does, which is the cleanup path below.
...
adoptedRow() returns the row so handle() can distinguish three cases: not yet processed (isImage() false) -> wait, leave alone; processed and still referenced -> resume, rewrite and delete via rewriteTo(); processed but no longer referenced -> keep, and let app:prune-orphaned-media collect the row as ordinary unowned media (it will: the row has no owner, and no body names it). Never re-run processInto() on a file that already has a row -- that is the duplicate-row/staged-copy leak the guard was added for, and a test asserts three runs yield one row.

rewriteBody() replaces the host-prefixed /storage/ form (any host, protocol-relative included) BEFORE the bare /storage/ form -- an absolute URL contains the bare one, so the bare-first order splices the old host onto the front of the absolute replacement URL and then deletes the original the page still names. rewriteTo() re-runs pagesReferencing() after the rewrite and deletes the original only when it returns empty, so an unmatched form costs one more run rather than a broken image on a live page.

The orphan retention window must EXCEED the adopter's daily cadence: between adoption and the rewrite, an adopted row is ownerless and not yet body-referenced (the body still names the original in page-body/), so its survival rests entirely on that window. A MEDIA_ORPHAN_RETENTION_HOURS below 24 lets the hourly prune delete every not-yet-rewritten adoption and the command loops forever without closing the EXIF gap.
