...
// The storefront. 'videos', 'cart' and 'orders' are in Page::RESERVED_SLUGS for
// the same reason 'contact' is: a content page there would be unreachable.
Route::livewire('videos', Catalogue::class)->name('shop.index');
Route::livewire('videos/{package}', ShowPackage::class)->name('shop.show');
Route::livewire('cart', Basket::class)->name('cart');

// Signed because customers check out without an account: the signature, not a
// login, is what keeps one customer's order confirmation from another.
